...
export const MultiRangeSlider = ({min, max, onChange}) => {
  ...
  const minValueHandel = (e) =>{
    ...
    setStyle({left:`${Math.round((+e.target.value*100)/max)}%`, width:`${Math.round((((+value.max) - (+e.target.value)) *100)/max)}%`})
  ...
  const maxValueHandel = (e)=>{
    ...
    setStyle({...style, width:`${Math.round((+e.target.value*100)/200) - +style.left.replace("%", "")}%`})
  ...
  return (
    <div id="multi-range-slider-wrap">
      <div className="multi-range-slider-thumb--wrapper">
        <input className="multi-range-slider-thumb multi-range-slider-thumb__left" onChange={minValueHandel} value={+value.max > +value.min && value.min} type="range"  min={min} max={max}/>
        <input className="multi-range-slider-thumb multi-range-slider-thumb__right" onChange={maxValueHandel} value={+value.min < +value.max && value.max } type="range" min={min} max={max}/>
